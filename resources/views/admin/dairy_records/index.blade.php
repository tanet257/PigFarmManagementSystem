        ...
        <form method="GET" action="{{ route('dairy_records.index') }}" class="d-flex align-items-center gap-2 flex-wrap">
            ...
            <div class="dropdown">
                ...
                <ul class="dropdown-menu">
                    <li><a class="dropdown-item" href="{{ route('dairy_records.index', array_merge(request()->except('farm_id'), [])) }}">ฟาร์มทั้งหมด</a></li>
            ...
            <div class="dropdown">
                ...
                <ul class="dropdown-menu">
                    <li><a class="dropdown-item" href="{{ route('dairy_records.index', array_merge(request()->except('batch_id'), [])) }}">รุ่นทั้งหมด</a></li>
            ...
            <div class="dropdown">
                <button class="btn btn-sm dropdown-toggle" type="button" data-bs-toggle="dropdown" 
                    ...
                    <i class="bi bi-house"></i> {{ request('barn_id') ? ($barns->find(request('barn_id'))->barn_code ?? 'เล้า') : 'เล้าทั้งหมด' }}
                </button>
                <ul class="dropdown-menu">
                    <li><a class="dropdown-item" href="{{ route('dairy_records.index', array_merge(request()->except('barn_id'), [])) }}">เล้าทั้งหมด</a></li>
            ...
            <div class="dropdown">
                <button class="btn btn-sm dropdown-toggle" type="button" data-bs-toggle="dropdown" 
                    ...
                    @if(request('type') == 'food') อาหาร
                    @elseif(request('type') == 'treatment') การรักษา
                    @elseif(request('type') == 'death') หมูตาย
                    @else ประเภททั้งหมด
                    @endif
                </button>
                <ul class="dropdown-menu">
                    <li><a class="dropdown-item" href="{{ route('dairy_records.index', array_merge(request()->except('type'), [])) }}">ประเภททั้งหมด</a></li>
                    <li><a class="dropdown-item {{ request('type') == 'food' ? 'active' : '' }}" 
                        href="{{ route('dairy_records.index', array_merge(request()->all(), ['type' => 'food'])) }}">อาหาร</a></li>
                    <li><a class="dropdown-item {{ request('type') == 'treatment' ? 'active' : '' }}" 
                        href="{{ route('dairy_records.index', array_merge(request()->all(), ['type' => 'treatment'])) }}">การรักษา</a></li>
                    <li><a class="dropdown-item {{ request('type') == 'death' ? 'active' : '' }}" 
                        href="{{ route('dairy_records.index', array_merge(request()->all(), ['type' => 'death'])) }}">หมูตาย</a></li>
                </ul>
            </div>
            ...
            <div class="dropdown">
                <button class="btn btn-sm dropdown-toggle" type="button" data-bs-toggle="dropdown" 
                    ...
                    @if(request('sort_by') == 'created_at') สร้างเมื่อ
                    @elseif(request('sort_by') == 'updated_at') แก้ไขล่าสุด
                    @else วันที่
                    @endif
                </button>
                <ul class="dropdown-menu">
                    <li><a class="dropdown-item {{ request('sort_by') == 'updated_at' ? 'active' : '' }}" 
                        href="{{ route('dairy_records.index', array_merge(request()->all(), ['sort_by' => 'updated_at'])) }}">วันที่</a></li>
                    <li><a class="dropdown-item {{ request('sort_by') == 'created_at' ? 'active' : '' }}" 
                        href="{{ route('dairy_records.index', array_merge(request()->all(), ['sort_by' => 'created_at'])) }}">สร้างเมื่อ</a></li>
                </ul>
            </div>
